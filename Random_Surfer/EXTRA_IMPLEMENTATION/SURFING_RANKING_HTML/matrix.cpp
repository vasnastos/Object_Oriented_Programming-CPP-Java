...
void matrix::find_rank()
{
    ...
    const int size=std::accumulate(this->visits,this->visits+this->rows,0);
     for(int i=0;i<this->rows;i++)
    {
        //Εύρεση για κάθε κορυφή του βαθμού της και εισαγωγή στην ουρά.
         this->rank.push(pagerank{i,(double)this->visits[i]/size});
    }
}
...
void matrix::print_ranking()
{
    ...
    matrix::rnk="<br><hr>";
    matrix::rnk="<table border=\"3\" style=\"text-align:center; font-size:16px; font-weight:bold; overflow:hidden; width:70%; color:#332531; background-color:#cfc29f;\">";
    matrix::rnk+="<tr style=\"background-color:green; color:blue; font-size:17px;\"><td>VERTEX</td><td>RANKING</td></tr>";
    while(!this->rank.empty())
    {
        std::cout<<"\t   "<<this->rank.top().vertex<<"\t"<<this->rank.top().rank<<std::endl;
        matrix::rnk+="<tr><td>"+std::to_string(this->rank.top().vertex)+"</td><td>"+std::to_string(this->rank.top().rank)+"</td></tr>";
        this->rank.pop();
    }
    matrix::rnk+="</table></center></body></html>";
}
